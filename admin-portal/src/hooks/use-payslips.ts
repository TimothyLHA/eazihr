...
export function usePayslips() {
  ...
  const fetchPayslips = useCallback(async () => {
    ...
    try {
      ...
      const { data: rawPayslips, error: payslipsErr } = await supabase
        .from('payslips')
        .select('id, employee_id, month, year, gross_pay, deductions, net_pay, status')
      ...
      const mapped: PayslipEntry[] = payslips.map((p) => {
        ...
        return {
          ...
          period: `${(p.month as number)}/${p.year as number}`,
  ...
  return { entries, stats, loading, error, refetch: fetchPayslips }
}
